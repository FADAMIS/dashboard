...
<body class="w-full h-screen justify">
    ...
    <div class="flex justify-center">
        ...
        <div class="grid grid-cols-3 gap-12 mb-10 w-full p-20 place-content-center"> 
            ...
            <button style="background-image: url({meal.image_path}); background-size: cover; background-position: center;" class="aspect-square basis-1/3 bg- rounded-3xl text-center font-mono text-3xl text-white " on:click={() => sendMeal(meal.name)}>{meal.name}</button>
            {/each}
        </div>
        {:else}
